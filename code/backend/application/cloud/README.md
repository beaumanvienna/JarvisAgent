...
| `cloudCircuitBreaker` | Per-connection circuit breaker (Closed/Open/HalfOpen).  `RecordFailure(name, ConnectorErrorCode)` stores the latest typed code per circuit; `ConnectionHealth::m_LastFailureCode` surfaces it via `GetHealthSummary` for `/api/status::connection_health[].last_failure_code`.  Failure-class policy via `IsConnectionFailure(code)` (in `connectorError.h`): connection-class codes (Network / Auth / Credential / OAuth / Http / InvalidConfig / InvalidEndpoint / Unknown) tick the consecutive-failure counter; `ValueOutOfRange` — app-level cap rejections where the connection is healthy and the call reached its endpoint (db_query `max_rows` / `max_output_bytes` / `statement_timeout`; storage upload-size caps) — is stored for display but skipped from the counter so a user hitting an app-level cap repeatedly is not self-locked-out.  Every cloud-task executor sets a typed `TaskState::m_LastFailureCode` on its `ExecuteCloud` failure paths; an absent code defaults to connection-class as a safety net. |
...
| Path | Purpose |
|------|---------|
| `code/backend/engine/keys/keyManager.h` | Credential storage and retrieval |
| `code/backend/engine/keys/oauthTokenManager.h` | OAuth2 token lifecycle and background refresh |
| `code/backend/engine/keys/jwtGenerator.h` | RSA RS256 JWT creation (Snowflake, service accounts) |
| `code/backend/engine/curlWrapper/awsSigV4.h` | AWS SigV4 signing (used by `s3Connector`, `azureBlobConnector` for AWS-compatible flows) — takes `SecureString const&` for secret_access_key + session_token |
| `code/backend/engine/curlWrapper/authSigner.h` | `IAuthSigner` polymorphic auth-header production (Bearer / x-api-key / Azure / SigV4 / Anthropic) — emits secret-bearing headers through a caller-owned `SecureString` slot, not a `std::string` vector |
| `code/backend/engine/curlWrapper/curlSlistHelper.h` | `AppendSecretHeader(curl_slist*&, prefix, SecureString const&, SecureString& scratch)` — canonical helper for inline `"Authorization: Bearer <secret>"` builds across cloud connectors / workflow filters.  Used wherever a connector talks to libcurl directly rather than via `IAuthSigner`.  See `doc/cyber security.md` "SecureString-only HTTP path". |
| `code/backend/engine/log/secretRedactor.h` | Scrubs secrets from log output |
| `code/backend/engine/core.h` | Owns `CloudConnectionManager`, `CloudConnectorRegistry`, `CloudCircuitBreaker`, `OAuthTokenManager` |
| `code/backend/application/web/webServer.cpp` | REST API endpoints for connections CRUD, OAuth flow, health status |
| `code/backend/application/workflow/triggerEngine.h` | Cloud trigger types (s3_watch, onedrive_watch, email_watch, azure_blob_watch, gcs_watch) |
