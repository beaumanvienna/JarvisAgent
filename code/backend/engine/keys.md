...
└── code/frontend/workflow-editor/ui/src/
    └── settings/
...
Uses vendored OpenSSL (already in `code/vendor/openssl`). No new dependencies.
...
```
code/backend/engine/keys/keyManager.h
code/backend/engine/keys/credential.h        — typed ICredential hierarchy (5 subtypes + factory)
```
...
1. ✅ Created `code/backend/engine/keys/keyManager.h` and `keyManager.cpp` — provider registry with CRUD.
2. ✅ Created `code/backend/engine/keys/keyEncryption.h` and `keyEncryption.cpp` — AES-256-GCM encrypt/decrypt
