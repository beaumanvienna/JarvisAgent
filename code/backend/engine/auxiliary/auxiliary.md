...
See `code/backend/application/file/README.md` for the use-site list (categorised by subsystem) and the documented opt-out cases (streaming writers with running caps, append-mode logs, operator-diagnostic dumps).
...
- `code/backend/engine/curlWrapper/awsSigV4.cpp::Sign` — canonical-request hash.
- `code/backend/application/web/mcpKeyManager.cpp` — MCP key + enrollment-token hashing.
- `code/backend/application/workflow/adhocWorkflowManager.cpp::SanitizeUserSlug` — slug-disambiguation suffix.
