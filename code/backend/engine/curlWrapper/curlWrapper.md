...
For secret-bearing headers built from a `SecureString`, prefer the standalone helper `AppendSecretHeader(curl_slist*&, prefix, SecureString const&, SecureString& scratch)` in `code/backend/engine/curlWrapper/curlSlistHelper.h` — single mlock'd allocation, no `std::string` intermediate, copy-and-swap exception safety.
...
- **`RateLimitController`** (`rateLimitController.h`) — per-`QuotaKey` adaptive controller. `ShouldAdmit(currentInflight, estimatedInputTokens)` projects the token bucket forward (deny if overshooting) and compares against the AIMD cap. `Observe(observation, was429)` updates state idempotently — known fields replace, unknown fields preserve, multiple calls per request produce identical state. AIMD: cap halves on 429, +1 every 5 clean completions, lower bound 1, upper bound from `config.rate_limit.max_concurrency`. **Time-based recovery** (the missing half of AIMD, cf. RFC 5681 "Restart of Idle Connections"): additive increase alone only fires on live successful traffic, so a cap reduced by a 429 would freeze below the ceiling forever once the load stops — penalising the next burst. Recovery is a pure function of the cap as of the last `Observe` (`m_CapAtIdleStart` / `m_LastActivityAt`) and the wall-clock elapsed since: the cap regains one slot per 10 s and a 60 s quiet period restores it straight to the ceiling. The baseline is advanced ONLY by `Observe`, never by a read — otherwise frequent dashboard polls would keep nudging the idle-restart deadline forward and it would never fire. Under load `Observe` runs every request so elapsed stays under the interval (pure AIMD, no time term); recovery only bites once traffic stops. Applied lazily on read (`ShouldAdmit` / `CurrentConcurrencyCap`, which mutate the `mutable` cap); serialised by the dispatcher's `m_DebugMutex`. Observability: `/api/debug/signals dispatcher_controllers[]` carries `current_concurrency_cap`, `hard_cap`, `last_429_at_ms` (0 = never actually throttled — a cap below `hard_cap` with no 429 is just AIMD ramping, NOT throttling), and `cap_recovery_eta_sec` (countdown to full recovery).
