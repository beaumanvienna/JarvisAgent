...
Scope: frontend (React workflow editor UI in `code/frontend/workflow-editor/ui/`).
...
See `todo.md` (project root) "Loose follow-ups" for the full description.  Frontend-only work; backend endpoints already in place.  Lift `code/frontend/dashboard/ui/src/components/MasterPasswordDialog.tsx` and the dashboard's `POST /api/auth/login` flow into a shared location, or duplicate into the editor.  Acceptance: restart j9t → open editor → master-password prompt appears with the same UX as the dashboard.
