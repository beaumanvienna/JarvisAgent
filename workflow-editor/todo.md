...
- [ ] Security log viewer in dashboard — expose `log/security.log` via `GET /api/log/security` (Engine mode, admin-only). Show in existing log viewer with `[security]` filter tab or dedicated panel.
