...
| File writes | `write_file`, `edit_file` require approval, route through `EngineCore::AtomicWriteFile` (`engine/auxiliary/file.h`) for the temp-write-then-rename, `.bak` backup taken before the helper call |
