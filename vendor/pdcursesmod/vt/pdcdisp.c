...
static void put_to_stdout(const char* buff, size_t bytes_out)
{
    ...
    while (bytes_out || (!buff && bytes_cached))
    {
        ...
        if (bytes_cached == TBUFF_SIZE || !buff)
            ...
            {
#ifdef _WIN32
                const int iwritten = _write(stdout_fd, tbuff, (unsigned int)bytes_cached);
#else
                const ssize_t iwritten = write(stdout_fd, tbuff, bytes_cached);
#endif
                if (iwritten <= 0)
                {
#ifndef _WIN32
...
#endif
                    break; /* real error or zero bytes — stop */
                }

                const size_t bytes_written = (size_t)iwritten;
...
void PDC_puts_to_stdout(const char* buff) { put_to_stdout(buff, (buff ? strlen(buff) : 1)); }
...
static size_t _unpack_combined_character(wchar_t* obuff, const size_t buffsize, const cchar_t ch)
{
...
static void color_string(char* otext, const PACKED_RGB rgb)
{
    ...
    else
    {
        ...
        if (red == green && red == blue) /* gray scale: indices from */
        {
            ...
            idx = ((blue - 35) / 40) + ((green - 35) / 40) * 6 + ((red - 35) / 40) * 36 + 16;
...
void PDC_doupdate(void) { put_to_stdout(NULL, 0); }
