...
- **`SecretRedactor::Redact` multi-secret prefix-collision over-leak** — Surfaced 2026-05-04 during S2 sitting 16.  `Redact()` iterates `m_Secrets` in registration order.  If secret A (e.g., `"abcd1234"`, 8 chars) is registered first and secret B (e.g., `"abcd12345678"`, 12 chars) second, a log line containing B has its first 8 bytes substituted to `[REDACTED]` by A's pass, leaving `"[REDACTED]5678"` — B's trailing 4 bytes leak.  Real registered credentials today are high-entropy distinct values so the collision is astronomically unlikely in practice, but it's a latent leak vector.  **Fix shape:** sort `m_Secrets` longest-first after every `AddSecret` (one-line `std::sort` in AddSecret with `[](auto& a, auto& b){ return a.size() > b.size(); }`); ensures the longer secret matches before any prefix-collision shorter secret runs.  Tiny correctness fix, but security-critical so it deserves its own deliberate landing rather than being bundled.
