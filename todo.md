...
### ~~[BUG, pre-1.0] Keyless local-LLM interface treated as "no provider" — blocks all ai_call workflows + false banner~~ — FIXED (working tree, 2026-06-06)
- **Remaining:** re-validate the keyless local-LLM from-scratch run on the CI-rebuilt 0.8.8 AppImage package (the fix is verified on the dev binary; see `hand-off.md` 2026-06-06 for the change list).
...
### Test rework — re-auth gate broke REST-integration provisioning
The keystore refactor (`8ed526c`) added a master-password re-auth gate on AI-interface + connection CRUD, but the REST-integration tests that *provision* interfaces/connections don't send `master_password`, so they now fail with `401 reauth_required` at setup — silently, since these need a running server (CI doesn't run them).  Confirmed broken: the whole `test/dispatch/*` suite (mock-interface provisioning, ~10 duplicated POST sites — `_stress_tui_helpers.py`, `_per_api_fault_helpers.py`, `test_chunking_fanout.py`, `test_bedrock_sigv4.py`, `test_relaxed_env_warnings.py`, `test_markitdown_cntx.py`, `test_cross_workflow_parallel.py`, `test_mock_dispatch_hermetic.py`, `test_s3_roundtrip.py`, `_tier_b_helpers.py`) + `test_edition_contract.py`.  Fix: thread `master_password` (from `--master-password` / `$JARVIS_MASTER_PASSWORD`) into every interface/connection create+delete — **ideally centralize the duplicated provision/teardown into one shared helper** (the POST sites violate extract-before-3rd-copy) so it's one place next time.  `test/security/test_url_policy.py` already fixed this way (2026-06-06, 25/25).  Acceptance: the dispatch suite runs green against a running instance.
