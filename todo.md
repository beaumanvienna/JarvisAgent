...
~~Pass 1 — rebuild the five curated example workflows by hand (`doc/misc/editor-dogfooding-plan.md`).~~ DONE: all 5 ☑; findings F-1…F-42 fixed, **zero deferred** (F-34 classified out-of-scope-by-design — render/semantic validation is a workflow the designer composes via `expose_error_signal` + a branch node, not an engine feature).  Remaining:
- **Editor UX refactor** — review: `doc/misc/editor-ux-refactor-plan.md` (no-code goal, principles, per-element proposals); development plan: `doc/misc/editor-ux-refactor-dev-plan.md` (code-level — abstractions U1–U7, per-phase file/function map, backend endpoints, round-trip/test strategy).  ~~Phase 0 (inspector hygiene + assists)~~ → ~~Phase 1 (files-as-nodes + hide paths)~~ → ~~Phase 2 (one data-carrying edge)~~ → ~~Phase 3 (no-code fan-out)~~.  **All four phases implemented — pending JC's e2e test + commit.**
  - ~~**Phase 0** — U7 typed extras · U6 `suggestOutputName`/`inferSchemaFromExample` + UI tiers · Filter-ID `<select>` · U5 Advanced `<details>` reorg~~ DONE.
  - **Phase 1** (foundations done; UI remaining):
    - ~~U4 `taskCapabilities.ts`~~ · ~~U1 `pathSynthesis.ts` (+ `.output.json`/per_item-glob fix)~~ · ~~backend `GET`/`POST /api/workflows/<id>/files`~~ DONE.
    - ~~U3 artifact-file node — `"file"` node kind + `FileNode.tsx` · `graphToJcwf` overlay-skip · `jcwfToGraph` reconstruction from `editor_layout` markers · `isFileNode` guard~~ DONE.
    - ~~File-node creation — API `uploadWorkflowFile`/`listWorkflowFiles` · "+ File" button + OS drag-drop (upload→place node) · `onConnect` file→task sets `file_inputs` + `file:` edge~~ DONE.
    - ~~Path-hiding (area E, display half) — a wired file_input shows its friendly source chip (📄 file / ← task) instead of the raw `../` path~~ DONE.
    - **Minor remaining:** "+ file" **picker** of already-uploaded files (`listWorkflowFiles` is wired in the API but no picker modal yet — upload is the primary path). `working_directory` is in the Advanced `<details>` (Phase 0) but not yet fully auto-hidden.
  - **Phase 2** (started):
    - ~~Golden round-trip test (idempotency + path-mirror contract, all 5 examples)~~ DONE — `code/frontend/workflow-editor/ui/test/goldenRoundtrip.mts` (`npx tsx test/goldenRoundtrip.mts`).
    - ~~U1 save-time derivation~~ DONE — `graphToJcwf` derives `file_inputs` from edges (edge-authoritative, S2 transition rule); fixed `deriveUpstreamOutputPaths` to mirror the backend `TaskPathResolver` base-leaf-strip (`resolveTaskDirSegments`); verified byte-identical `file_inputs` on all 5 examples incl. aiZipDemo.
    - ~~U2 unified ports (S5)~~ DONE — `editor/edgeClassify.ts::classifyEdge` is the single classifier used by `onConnect` + `graphToJcwf` (serialization + cycle detection); no handle-id migration; visual layer already shares the inline-handle styling (color kept distinct by design).
    - ~~B full (edge-implied filter binding)~~ DONE — `onConnect` filter→task sets `task.filter`+`per_item` + draws the fanout edge; the Filter `<select>` collapses to a read-only chip when an edge implies it. Round-trip verified.
    - ~~Optional cleanup: deleted the redundant stored-path sync branches in `updateSelectedTaskField`~~ DONE (save-time derivation supersedes them; golden+contract test still 10/10).
  - ~~**Phase 3** (no-code fan-out builder, area J)~~ DONE — `editor/buildFanoutBinding.ts` (S6) + `FanoutBuilder.tsx` (column checkboxes → live row-1 preview → unique-per-row `prob_files`, kills F-40) + backend `GET /api/workflows/<id>/files/<path>` content endpoint (reads the filter CSV header).
  - ~~Deferred follow-ups~~ DONE: "+ file" picker (`FilePickerDialog`) · `working_directory` friendly "runs in" hint · retroactive file-node inference (S3 — `jcwfToGraph` infers file nodes for external `file_inputs` inside the workflow folder; make-example now shows its 4 source files; verified lossless via golden+contract test). `jcwfToGraph(jcwf, workflowId?)` + `fileNodeInputPath(…, wfId)` signatures changed.
  - **Phase-0–3 refactor code-complete.** Remaining = JC's end-to-end browser test → commit (plans + implementation together; includes a backend rebuild).
  - ~~**Dev-plan §4b seams** — `dependency-graph` 404 (static route now registers before `/<string>`, since Crow picks the lowest rule index, not static-over-param) · validation `taskId` (the four dataflow findings now carry `taskId`; backend reliably sets it on every task-scoped finding, so the editor's dead `Task '…'` message-regex fallback is removed) · `/api/files/check` kept (resolves at project-root scope via `TaskPathResolver`, incl. out-of-folder cross-task paths the workflow-folder list cannot see — not subsumable).~~ DONE.
...
DONE: ran the system-level STRIDE pass (data-flow + trust-boundary + data-at-rest inventory, boundaries 1–11 + data stores).  No CRIT/HIGH; no open findings; six accepted risks (AR-1…AR-6).  Output is `doc/threat-model.md` (auditor-facing); methodology in `doc/misc/stride-pass-dev-plan.md`; cross-ref in `doc/cyber security.md`.
...
**Sequencing (JC, 2026-06-07):** do this only AFTER editor + assistant dogfooding are done — not now.  No full mac/Windows regression before then.  Claude reminds when dogfooding closes.
