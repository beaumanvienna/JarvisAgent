...
| `cloudSigV4::Sign` | `engine/curlWrapper/awsSigV4::Sign` driven with the S3 dispatch shape — `m_ContentSha256Override` + signed `Content-Type` extra header — exercises `m_ExtraHeadersToSign` and the runtime-sized `Build(std::span<...>)` path | 0 / 0 |
