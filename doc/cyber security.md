...
The AI-dispatch URL (`config.json::api_interfaces[].url`) follows its own scheme + address policy distinct from the cloud-connector SSRF gates above.  The reference implementation is `AIAssistant::UrlPolicy::ValidateAiInterfaceUrl` in `code/backend/application/network/urlPolicy.{h,cpp}`; hooks live at config-load (`ConfigParser::ParseInterfaces`) and REST CRUD (`HandleAiInterfaceCreatePost` / `HandleAiInterfaceUpdatePut`).
