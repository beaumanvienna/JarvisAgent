...
The AI-dispatch URL (`config.json::api_interfaces[].url`) follows its own scheme + address policy distinct from the cloud-connector SSRF gates above.  The reference implementation is `AIAssistant::UrlPolicy::ValidateAiInterfaceUrl` in `application/network/urlPolicy.{h,cpp}`; hooks live at config-load (`ConfigParser::ParseInterfaces`) and REST CRUD (`HandleAiInterfaceCreatePost` / `HandleAiInterfaceUpdatePut`).

- **Plain `http://` is loopback-only.**  An `http://` URL is accepted only when every address returned by `getaddrinfo` falls in the IPv4 loopback range `127.0.0.0/8` or is the IPv6 loopback `::1` (IPv4-mapped IPv6 `::ffff:127.x.y.z` normalised).  Multi-record DNS responses must ALL be loopback — closes the DNS-rebinding-style attack where a hostname returns both `127.0.0.1` and a routable IP.  The supported use case is local LLM dispatch (ollama / llama.cpp / vLLM at `http://localhost:11434`); anything else would carry prompts (potentially containing PII or template-substituted secrets) in clear text.  `https://` URLs pass without further URL-policy checks — transport encryption + libcurl's own DNS / TLS path apply.
...
- **Empty + whitespace credential rejection at the signer.**  Each `Apply` validates the credential is non-empty AND has at least one non-whitespace character via the `IsBlank` helper.  An accidentally-blank credential (`""` or `"   "` from a hand-edit of `keys.json` or a misconfigured env var) is caught locally with a structured ERROR, rather than going on the wire and bouncing off the provider as an opaque 401.  SigV4's three required fields (`access_key_id`, `secret_access_key`, `region`) are read from the typed `QueryData::m_AwsCredential` snapshot and validated independently so a missing region produces a different error than a missing secret.  The signer ALSO fails closed if `m_AwsCredential` itself is null on the SigV4 path — a defensive backstop in case a caller routes a request to SigV4 without populating the typed credential.
