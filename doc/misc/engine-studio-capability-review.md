...
- ~~**Dashboard "Run" button visibility on Engine.**~~ ✅ Verified working (JC, 2026-04-28) — Run button appears for operator+ users on the Engine dashboard.
- ~~**AI-WebSocket dispatch extraction.**~~ ✅ Landed in Sitting 4 of `pre-1_0_follow-ups.md` (2026-05-18) — `WebServer::HandleAssistantWebSocketMessage` declared in `webServer.h`, Studio impl in `webServer_studio.cpp`, no-op Engine impl in new `webServer_engine.cpp` (file-level edition isolation via premake `removefiles` in both directions).  `#ifdef J9T_STUDIO` count in `webServer.cpp`: 8 → 5.
- ~~**`HandleAiInterfaceTestPost` Engine fallback.**~~ ✅ Landed in Sitting 4 of `pre-1_0_follow-ups.md` (2026-05-18) — option (a) per the original recommendation: `AiRequestPool::TestInterface` is the new home of the synchronous probe, available in both editions.  The `#ifdef J9T_STUDIO ... ai_test_not_available_in_engine ... #endif` block is gone; Engine admins use the dashboard Test button against live providers.
...
- ~~**`HandleAiInterfaceTestPost` location.**~~ ✅ Resolved by the same Sitting 4 change above — the inline `#ifdef J9T_STUDIO` around the old `m_AiJcwfService.TestAiInterface(...)` call no longer exists.  Handler stays in `webServer.cpp` (common file); calls `AiRequestPool::TestInterface` unconditionally.
