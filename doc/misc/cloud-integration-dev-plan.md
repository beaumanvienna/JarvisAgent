...
- [x] Frontend: Redmine connection config (endpoint + key_name) in `shared-ui/views/ConnectionsView.tsx` + task inspector section in `workflow-editor/ui/src/editor/WorkflowEditorView.tsx`; `redmine_issue` listed in `JcwfTaskType`.
...
- [x] **Jira round-trip** (`jiraIssueDemo`): AI generates bug report → `jira_issue/create` with `description_file` piped from `{{ai_bug_report.output_file}}` → `jira_issue/get` with `issue_key={{create_issue.json.key}}` → python `verify` task asserts key match, summary, issue type, labels, and AI text in retrieved ADF description. Verified against real Jira Cloud tenant, issue SCRUM-1 created and retrieved successfully. Introduced `{{taskId.json.<path>}}` template resolution (flattened from upstream `response.json`) and per-child `response_<N>.json` naming for concurrent per-item cloud executors.
- [x] **Google Sheets round-trip** (`sheetsQuizGrader`): `sheets_read` pulls quiz questions + student answers → `ai_call` grades each answer and emits CSV via the `outputs` slot pattern (not `file_outputs`) → `sheets_write` uploads grades to columns C/D/E via `{{grade_answers.output_file}}`. Verified end-to-end against a real Google Sheet with the full OAuth2 flow. Also generalized the `/api/connections/<name>/oauth/authorize` and `/oauth/callback` handlers to work with any connector (new `OAuth2ProviderInfo` virtual on `ICloudConnector`, implemented for OneDrive + Google; previous code hardcoded OneDrive/Microsoft). Fixed pre-existing PKCE bugs (code_verifier was 32 chars instead of ≥43, code_challenge was not proper base64url). Fixed redirect URI scheme to follow TLS config. Added OAuth token persistence: refresh_token + token_endpoint + client_id + client_secret are stored encrypted in `keys.json.enc`, hydrated on `OAuthTokenManager::Start`, and refreshed on demand in `GetAccessToken` — surviving j9t restarts without re-consent.
- [x] **OneDrive round-trip** (`oneDriveUploadDownloadDemo`): `onedrive_upload` ships a seed `status.txt` from inside the workflow folder → `onedrive_download` fetches it back → `ai_call` (PM persona, cntx_files=[downloaded_status.txt]) generates a project-health review → `onedrive_upload` writes the AI review back to `j9t-demo/ai_review.txt` via `{{ai_review.output_file}}`. Verified end-to-end against real OneDrive (`jc.lache@gmail.com`) using a fresh Azure free-trial tenant + Entra ID app registration with **Mobile and desktop applications** platform (PKCE-only, no client secret) and **Allow public client flows = Yes**. OAuth refresh token persisted encrypted to `keys.json.enc` and the OAuth callback now auto-creates the matching KeyManager provider on first authorize (previously the provider had to exist beforehand or tokens stayed memory-only); restart-survival verified by killing j9t, relaunching, and re-running the workflow without re-consent — including a fresh-tenant test where the provider was deleted before authorize. Replaced the prior upload-only demo with a true round-trip JCWF (added `02_download` + `03_ai_review` + `04_upload_review` stages).
- [x] **Snowflake round-trip** (`snowflakeQueryDemo`): create database + tables → insert 8 sales records → query aggregated region stats to CSV → per-item AI classifier labels each region STRONG/MODERATE/WEAK → per-item INSERT verdict back into a dedicated analysis table → final `verify` task SELECTs row counts. RSA JWT (RS256, KEYPAIR_JWT auth) + Snowflake SQL REST API async polling (`POST /api/v2/statements` → handle, `GET /api/v2/statements/{handle}` until completion). Verified end-to-end against a real Snowflake trial; see `example/workflows/snowflakeQueryDemo.md`.
- [x] **Slack round-trip** (`slackQAndABot`): `slack_read` fetches recent non-bot messages from `#j9t-demo` via `conversations.history` → `ai_call` answers the most recent user question as a C++/Vulkan expert (cntx_files pointing at `latest_message.txt`) → `slack_message` posts the answer as a threaded reply using `text_file` + `thread_ts_file`. Introduces `TaskType::SlackRead`, `exclude_bots` filter, and `thread_ts`/`text_file`/`thread_ts_file` params on `slack_message` (mirrors the `email_send` body_file pattern). Requires bot scopes `chat:write` + `channels:history` + `channels:read`. Replaces the prior one-way `slackMessageDemo` notification demo. Verified end-to-end against real Slack workspace `j9t-test`: user asked *"What is the purpose of vkCreateFramebuffer in Vulkan?"*, bot replied in-thread with a ~200-word technical answer covering attachments, `VkFramebufferCreateInfo`, and `vkCmdBeginRenderPass` semantics.
