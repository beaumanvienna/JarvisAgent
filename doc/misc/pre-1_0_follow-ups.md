...
**Sitting 16 Part 1 2026-05-24** — first pass on the basket: items #1 (SigV4 consolidation), #2 (StoreTokens SecureString widen), #3 (HmacSha256 string_view), #4 (MCP heartbeat 503 on locked keystore) all closed.  Verified by Bedrock SigV4 KAT (signature `1a6d6607...` unchanged) + heap-scan audit (10/10 PASS) + 225+ regression checks.  3 new deferred-verification items added (#12 live S3 smoke, #13 live OAuth round-trip, #14 pre-existing heartbeat-handler `auth->m_Error` check) — uncovered during verification; not blocking the original 4 closures.  **7 original + 3 new items remain in the basket** (#5–11, #12–14).  See `doc/misc/hand-off.md` 2026-05-24 Sitting 16 Part 1 entry.

**Sitting 16 Part 2 2026-05-24** — items #5 (Polarion path-traversal coverage via existing polarionMockup at `:18080`) and #6 (AI output 64 KiB cap coverage + UTF-8 truncation fix) closed.  #5 added three sub-tests to `test/hardening/test_negative_paths.py` Group 2: 2.4 (DownloadAttachment outputPath rejection, fires pre-network), 2.5 (IsValidPolarionId `..` rejection), 2.6 (WriteItemFile IsValidFilesystemId allowlist via mock-returned items) — Group 2 went 7→20 PASS.  #6 extracted `BuildCallbackPayload` from `FireCompletionCallback` into a public free function, added a Debug-only endpoint `GET /api/debug/build-callback-payload?runId=X` so tests can inspect the payload without the SSRF gate's loopback rejection getting in the way, and fixed a latent UTF-8 truncation bug: caller now reads `kMaxOutputBytes + 3` slop bytes so `TruncateUtf8Safe` sees the straddling multibyte sequence and backs off to a complete codepoint boundary — without the slop, an exactly-at-cap byte-level read leaves a dangling lead byte (invalid UTF-8 that detonates strict JSON validators and `PyUnicode_FromString` on Python receivers).  Group 1 went 4→12 PASS; full negative-paths suite 41→49 PASS; `test_auth_mcp` 100/100.  New `scripts/writeOutputCapCanary.py` helper writes the 65538-byte canary (65535 'A' + 3-byte UTF-8 '€') that puts the codepoint precisely across the 65536-byte boundary; test asserts callback payload content is exactly 65535 bytes (partial '€' dropped) and well-formed UTF-8.  `doc/api-endpoints.md` updated.  **8 items remain in the basket** (#7–11, #12–14).  See `doc/misc/hand-off.md` 2026-05-24 Sitting 16 Part 2 entry.

**Sitting 16 Parts 3–6 2026-05-24 → 2026-06-01** — Part 3 closed #7 (reaper shutdown-window teardown), #8 (circuit-breaker `IsConnectionFailure` policy), #9 (`ReadMeta` simdjson); Part 4–5 closed #10 (email_watch UIDVALIDITY persistence + integrity guards, fully automated greenmail test).  **Part 6 closed the entire remaining basket** (#11 KeyManager cap synth tests, #12 live S3 round-trip, #13 live OAuth consent round-trip, #14 heartbeat `auth->m_Error`, #15 all 12 cloud executors → typed `ConnectorErrorCode`, #16 reaper CV-mutex + Reap-first, #17 watermark prune-on-removal) plus the `triggerEngine.cpp:521` drive-by.  Verified on the merged tree: 4-config build clean, heap-scan 9/0/1 PASS, full live test sweep green (auth-mcp 103, negative-paths 55, email-watch 19, keymanager-caps 4, s3-roundtrip 6, url-policy 27, slug 14, malformed-configs 56, SigV4 KAT).  **The Sitting-16 basket is now empty — every item closed.**  See `doc/misc/hand-off.md` 2026-06-01 entry.
...
**Sitting 16 grew by 3 items** during this sitting (the three deferred D1 items above).  See `doc/misc/hand-off.md` 2026-05-23 entry.
...
1. **Two `SigV4Signer` classes in `AIAssistant` namespace** — engine + cloud variants with parallel canonical-request implementations.
   - **Why it matters:** Two parallel canonical-request implementations are a guaranteed drift surface.
   - **Status:** closed 2026-05-24.  Engine signer extended (`m_ContentSha256Override` + `m_ExtraHeadersToSign`); cloud `sigV4Signer.{h,cpp}` deleted; `heapScan_cloud_scenarios.cpp` collapsed back into `heapScan_test.cpp`; Bedrock SigV4 KAT signature unchanged (`1a6d6607...`); heap-scan audit 10/10 PASS including new `cloudSigV4::Sign` scenario (S3 dispatch shape via engine signer) at 0/0.  Live S3 round-trip not exercised — tracked separately as item #12.

2. **`OAuthTokenManager::StoreTokens` takes `std::string const&` for three secret-bearing params** — `accessToken`, `refreshToken`, `clientSecret`.
   - **Why it matters:** Same SecureString-threading opportunity as `PerformRefresh`'s prior snapshot leak.
   - **Status:** closed 2026-05-24.  Signature widened to `SecureString const&` for all three secret params; consent-callback caller (`webServer.cpp` OAuth callback handler) builds SecureString locals from the simdjson views + `connection->m_Params["client_secret"]`.  Same threading shape as Sitting 8e R1's `PerformRefresh` snapshots.  Live OAuth round-trip not exercised — tracked separately as item #13.

3. **Engine `awsSigV4.cpp::HmacSha256` `data` parameter is `std::string const&`** — inconsistent with the cloud-side sibling which takes `std::string_view`.
   - **Why it matters:** Minor consistency / maintenance.  Not a security issue.
   - **Status:** closed 2026-05-24.  Param widened to `std::string_view`; SigV4 self-test PASSED + KAT signature unchanged.

4. **MCP-bridge heartbeat traps locked-keystore startup into auto-lockout** — heartbeat polling against a freshly-started (locked) j9t exhausts the 10-failures-per-5-min per-IP lockout budget within ~150 s.
   - **Why it matters:** Self-DoS during any routine restart (code change, reboot, crash recovery); during the lockout window REST shutdown is also blocked.
   - **Status:** closed 2026-05-24.  `HandleMcpHeartbeatPost` short-circuits with HTTP 503 + `error: keystore_locked` when `m_McpKeysLoaded` is false, BEFORE `TryMcpAuth` / `RecordAuthFailure`.  503 instead of 423 because Crow's response constructor rewrites 423 → 500.  Verified live: 12 consecutive heartbeats while locked → 0 entries in `log/security.txt`; post-unlock + invalid token still increments lockout (adversarial-flood protection preserved).  Pre-existing handler bug uncovered during verification (success path doesn't check `auth->m_Error`) tracked separately as item #14.  `doc/api-endpoints.md` updated with the new error code.

5. **Polarion `WriteItemFile` / `DownloadAttachment` path-traversal coverage** — Sitting 11 (D1 hardening pass) deferred this because polarion fixtures aren't trivially driveable from the host's running j9t.
   - **Why it matters:** completeness of the D1 hardening test matrix.  A test gap means a future regression in the containment gate would slip through CI.
   - **Status:** closed 2026-05-24 (Sitting 16 Part 2).  Three sub-tests added to `test/hardening/test_negative_paths.py` Group 2 against `../polarionMockup` at `http://localhost:18080`:
     - **2.4** — `polarion_write download_attachment` with `file_path=/tmp/...` → `ConfineUnderProjectRoot` rejects pre-network, task fails, no escape file written
     - **2.5** — same task with `work_item_id=../escape` → `IsValidPolarionId` allowlist rejects `..` substring
     - **2.6** — `polarion_query` with `filter.id=../escape` → mock returns work items, per-item `WriteItemFile` `IsValidFilesystemId` rejects, ERROR log line emitted with both `WriteItemFile rejected filter.m_Id '../escape'` and `fails allowlist`
     - Group 2: 7→20 PASS.  Mock required no changes (pagination + 18 seed items in `GoKartProcurement` covered the WriteItemFile branch).  The `WriteItemFile` ConfineUnderProjectRoot post-allowlist branch (only reachable via symlink rigging at `<workflowBaseDir>/<filter_id>`) intentionally not covered — too artificial to be worth the test rigging.

6. **AI output 64 KB size cap (kMaxOutputBytes) coverage + UTF-8 truncation fix** — Sitting 11 deferred because the cap applies to output file content read AFTER an AI call completes (the callback-payload renderer), and the truncation is internal — not REST-observable as a rejection.
   - **Why it matters:** completeness.  Internal truncation has a behaviour contract (cap at 64 KiB) that no test exercises today.  A regression that raises/lowers the cap would go undetected.
   - **Status:** closed 2026-05-24 (Sitting 16 Part 2).  Refactored `workflowRuntimeManager.cpp`: extracted `BuildCallbackPayload(WorkflowRun const&, bool includeOutputs) → std::string` as a public free function (declared in `workflowRuntimeManager.h`); `FireCompletionCallback` now calls it.  **UTF-8 bug fix:** caller reads `kMaxOutputBytes + 3` slop so `TruncateUtf8Safe` sees the straddling multibyte sequence and backs off to a complete codepoint boundary — without the slop, an exactly-at-cap byte-level read leaves a dangling lead byte (invalid UTF-8).  New Debug-only endpoint `GET /api/debug/build-callback-payload?runId=X&include_outputs=...` (admin-gated) so tests can inspect the rendered body without the SSRF gate refusing a loopback callbackUrl.  New helper `scripts/writeOutputCapCanary.py` (python task) writes a 65538-byte canary file (65535 'A' + 3-byte '€' straddling the boundary).  New sub-test 1.2 in `test/hardening/test_negative_paths.py` asserts callback payload content is exactly **65535 bytes** (partial '€' dropped) and well-formed UTF-8; skips on Release (probes endpoint first).  Group 1: 4→12 PASS; full negative-paths suite 41→49; `test_auth_mcp` 100/100.  `doc/api-endpoints.md` updated with the new debug endpoint.
...
7. **Reaper CV wake-on-stop shutdown-timing test** — Sitting 11 deferred because the test requires a controlled shutdown of a j9t instance and the host's running j9t can't be hit for this kind of timing measurement without disturbing other tests.
   ...
   - **Status:** closed 2026-05-24 (Sitting 16 Part 3) — addressed structurally rather than by a perf-based test.  Investigation found the reaper teardown was running during `~AdhocWorkflowManager` in the `unique_ptr<Application>` destruction at end of `main`, AFTER `engine.cpp`'s 3-s shutdown watchdog had been diffused — so any future CV-wake regression would have silently stalled exit by up to 60 s with no watchdog line and no force-exit.  Fix: lifted `m_AdhocManager->StopReaperThread()` into `WebServer::SignalStop()` (just after the WRM observer detach), bringing reaper teardown inside the watchdog window.  Now any stall regression here triggers `_exit(EXIT_FAILURE)` with the `[shutdown watchdog] timeout expired` line — a stronger guarantee than the proposed perf test would have provided.  Verified end-to-end: REAPER-INST instrumentation showed `StopReaperThread` total 182 µs inside Phase 1, with `wait_for` returning via `predicate-true(stop)` (the CV wake path).  Companion audit of all long-lived threads (Studio + Engine) confirmed zero remaining UNSAFE join sites — every `std::thread`/`std::future`/threadpool worker is now joined inside the watchdog window.  Original git claim of "60 s blocking" was inaccurate too — the pre-CV implementation (commit 41c325b) used 60×1s chunked-poll, so worst case was ≤ 1 s, not 60 s.
...
8. **Circuit breaker counts expected app-level failures as "connection failures"** — `CloudCircuitBreaker::RecordFailure` is called by every task-level rejection in the cloud-connector / db_query path, including expected-app-level outcomes like "Result set has 100 rows, exceeds max_rows=5" (db_query cap) or "exceeds statement_timeout".  Five consecutive such rejections trip the breaker open and subsequent requests short-circuit with `"Cloud connection 'X' circuit breaker is open"` instead of reaching the actual database.  Surfaced by Sitting 11's Group 4 hardening tests — they need a `_warm_up_connection` helper that calls `POST /api/connections/<name>/test` before each cap-rejection assertion, otherwise consecutive tests trip the breaker open and the assertions short-circuit.
   ...
   - **Status:** closed 2026-05-24 (Sitting 16 Part 3).  Implemented option C from the sitting brief — typed `IsConnectionFailure(ConnectorErrorCode)` policy helper.  Changes: (a) new `ConnectorErrorCode::ValueOutOfRange` variant for known-good-connection app-level rejections; (b) `[[nodiscard]] bool IsConnectionFailure(code)` in `connectorError.{h,cpp}` — switch is `default:`-free per CLAUDE.md, every variant enumerated (connection-class for InvalidConfig / InvalidEndpoint / CredentialMissing / CredentialInvalid / OAuthError / NetworkError / AuthFailure / HttpError / UnknownError; app-level for ValueOutOfRange); (c) new `std::optional<ConnectorErrorCode> TaskState::m_LastFailureCode` (sibling of `m_LastErrorMessage` in `workflowTypes.h`) — clean seam, no virtual signature changes across the 13 cloud-task subclasses; (d) base `ICloudTaskExecutor::Execute` passes `taskState.m_LastFailureCode` to `RecordFailure`; (e) `dbQueryCloudTaskExecutor.cpp` populates the field at the three cap-rejection sites (row cap, byte cap, statement_timeout firing via SQLSTATE 57014 / `PG_DIAG_SQLSTATE` on the query-failed branch); (f) `CloudCircuitBreaker::RecordFailure` stores the typed code for display but skips counter+state-transitions when `IsConnectionFailure(*code) == false`.  Legacy untyped callers (no code) remain conservative-default connection-class so unmigrated subclasses preserve existing behaviour.  New test 4.4 in `test/hardening/test_negative_paths.py` submits N=6 consecutive max_rows cap rejections without warm-up and asserts breaker stays Closed, `consecutive_failures` unchanged from baseline, and `last_failure_code == "value_out_of_range"`.  Group 4: 12→18 PASS; full negative-paths suite 49→55; `test_auth_mcp` 100/100 regression.  Vestigial `_warm_up_connection` calls in tests 4.2/4.3 left in place — they're now no-ops for the cap branches but harmless defensive bridge while other cloud connectors haven't yet emitted typed app-level codes.
...
9. **`AdhocWorkflowManager::ReadMeta` uses hand-rolled JSON `pluck` instead of simdjson** — `adhocWorkflowManager.cpp:373-394` extracts four fields from `meta.json` via `std::string::find`-based substring scans without JSON-unescape.  Theoretical-only weakness today (the corresponding `WriteMeta` uses `JsonHelper::EscapeJsonString` so escapes are introduced symmetrically, and the four field values are all upstream-validated by McpKeyManager / the policy whitelist), but it's a hand-rolled JSON parser in a security-critical attribution path — violates `feedback_simdjson_only`.  Surfaced during the Sitting 9 doc sweep on 2026-05-23.
   - **Status:** closed 2026-05-24 (Sitting 16 Part 3).  Replaced the `pluck` lambda + four `pluck("...")` calls with `simdjson::dom::parser` + `parser.parse(content)` + `root[field].get_string()` — mirrors the existing `RewriteWorkflowId` idiom in the same file.  Parse failure (truncated/corrupt JSON) or non-object root logs at ERROR with the meta path; missing file (the normal "no meta" hot path on Init's filesystem scan) still returns nullopt without logging.  The parent-dir-name fallback for legacy meta files lacking `owner_slug` is preserved as before.  Regression: `test_auth_mcp` 100/100, `test_adhoc_user_slug_collision` 14/14, `test_negative_paths` 55/55 — adhoc submission + per-user authz lookups + slug-collision e2e all route through `ReadMeta` and continue to pass end-to-end.
...
10. **email_watch save-path end-to-end test** — the load + restore paths were verified live (planted sentinel → restart → load INFO + restore INFO fires) but the save path (poll completes → `.email_watermarks.json` atomic-rewritten with the new UID) was code-review only because the host's `my-email` connection has an empty endpoint; the IMAP target validator rejects before any successful poll fires the save.
   ...
   - **Status:** closed 2026-05-25 (Sitting 16 Part 5).  Two phases: Part 4 landed the most-correct redesign (option D from the sitting brief) — `STATUS folder (UIDVALIDITY)` IMAP request, `format_version: 2` with `connection_name` / `folder` / `uid_validity` as load-bearing fields, registration-time `(connection, folder)` guard, poll-time UIDVALIDITY-change guard, v1-file rejection per `feedback_no_legacy`.  Part 5 closed the test residual.  Investigation surfaced two real integrity gaps the original "save mirrors load" framing would NOT have caught: (a) the lookup key was `(workflow_id, trigger_id)` and `connection_name` / `folder` were marked "informational" — editing a JCWF to repoint a trigger to a different IMAP server would silently apply the old server's UID to the new one (UIDs are only meaningful within `(server, mailbox-UIDVALIDITY)`); (b) UIDVALIDITY (RFC 3501 §2.3.1.1) was not tracked at all — a mailbox renumber would silently misapply old UIDs to new messages.  Both fixed in Part 4.
   - **What's verified.**  `test/hardening/test_email_watch_persistence.py` now owns the j9t restart cycle (REST shutdown → relaunch via `jarvisagent.sh` → `JARVIS_MASTER_PASSWORD` unlock) and runs five fully-automated scenarios against the live greenmail container (SMTP 3025 / IMAP 3143, user `test:test`).  Aggregate **16 assertions / 0 failures**: **S1** save-with-UIDVALIDITY (6 — format_version=2, connection_name, folder, uid_validity, last_seen_uid, updated_at); **S2** v1-file rejection across restart (2 — load-time WARN substring + file-shape post-load); **S3** registration-time `(connection, folder)` guard (1 — `discarding stale watermark for trigger` WARN with both targets named); **S5** restore happy-path (4 — `restored persisted UID watermark '<u1>'` + `firing trigger workflow='<wfid>' trigger='ew'` + watermark advanced U1→U2 + UIDVALIDITY unchanged); **S4** poll-time UIDVALIDITY-change guard via `docker restart greenmail` (3 — `UIDVALIDITY changed (V1 -> V2)` WARN + absence of fire-trigger line + watermark `uid_validity` updated V1→V2).  Regression: `test_negative_paths` 47/47, `test_auth_mcp` 100/100.
   - **Gotchas next-session-Claude should know.**
     - **greenmail SMTP routing requires the bare username `test`** as the To-address — `-Dgreenmail.users=test:test` creates a single user whose email IS `test` (no @-domain).  Mail to `test@anything` auto-creates a different user and is invisible to the `test` IMAP login.  Empirically `test@example.com` worked in older runs because of a stale auto-created user; after any greenmail restart only the bare-username route survives.
     - **Shell tasks enforce a `scripts/` prefix** (`shellTaskExecutor.cpp:840-1162`) — `touch /tmp/...` is rejected at executor entry per `feedback_allowlist_not_blocklist`.  The test exercises fires via the log line `[email_watch] new mail detected ... firing trigger workflow='<wfid>'` rather than a marker file; the canvas task is `scripts/echo.sh` (sanctioned no-op).
     - **log.txt is recreated on every j9t restart** (spdlog opens fresh) — offset-based readers post-restart need to fall back to offset 0 when the file is smaller than the captured offset (the test handles this in `wait_for_log_substring` and `read_log_tail`).
     - **Drive-by bug in `triggerEngine.cpp:521`** — the AddEmailWatchTrigger INFO log uses `instance.m_Folder` AFTER `m_EmailWatchTriggers.push_back(std::move(instance))` (line 516), so the log line prints `folder=` empty.  Functionally harmless (the moved-into vector entry has the correct folder; the poll log lines show `folder 'INBOX'`).  Reorder the log call before the push, or capture the folder string into a local before the move.
...
11. **KeyManager hardening synthetic tests not written** — Sitting 14's plan listed two acceptance-criteria tests that were code-review verified only: (a) empty-string `SetDefaultProvider` → rejection + WARN log, (b) malformed 100k-provider keystore → parser rejection + LOG_CORE_ERROR.
   ...
   - **Status:** closed 2026-06-01 (Sitting 16 Part 6).  New `test/hardening/test_keymanager_caps.py` drives both caps on the **encrypted** `keys.json.enc` path (a subprocess sandbox spawns j9t and POSTs `/api/settings/keys/unlock` to trigger `Load`): T1 a >4 MB keystore → `exceeds kMaxKeysFileBytes` (refused pre-decrypt); T2 a Python-crafted VALID 1030-provider keystore (reproduces the `JKEY` AES-256-GCM wire format) → decrypts then `provider count exceeds kMaxProviders=1024`.  4/4 PASS, neither crashes.  The empty-name `SetDefaultProvider` WARN guard is defense-in-depth, unreachable via external input (REST `HandleProviderSetDefaultPost` gates on `HasCredential`; the keystore-load path assigns `m_DefaultProviderName` directly) — `[[nodiscard]]` + code review cover it; T3 (optional) verifies the set-default REST surface fails closed.  **Surfaced + fixed a cyber-sec-doc violation while here:** the Studio-only plaintext-`keys.json` load path (`KeyManager::LoadPlaintext`) and the `OPENAI_API_KEY` env-var credential fallback (`LoadFromEnvironment`) both contradicted the "same cyber-sec across editions / no unsecured keys / encrypted-at-rest" posture in `doc/cyber security.md` — both purged (decls + bodies + the `engine.cpp` `#ifdef J9T_STUDIO` block + env fallback), so `keys.json.enc` (runtime-unlocked) is now the sole credential source in every edition.  All credential docs (`engine/keys.md`, `doc/jarvisagent.md`, `doc/cyber security.md`) updated to state-of-the-art.

12. **Live S3 round-trip smoke for the consolidated signer** — added 2026-05-24 after item #1 closed.  Heap-scan + Bedrock KAT cover the canonical-request correctness, but no live request was exercised against a real S3 endpoint this session.
    - **Why it matters:** A regression in the engine signer's S3 dispatch shape (e.g. the new `m_ExtraHeadersToSign` path mis-sorting Content-Type) would be invisible to the existing tests — the heap-scan audit checks residue, the KAT locks the Bedrock signature only, and the regression suite doesn't exercise live S3.
    - **Fix sketch:** Configure an S3 connection (AWS or LocalStack); submit a JCWF that runs list / upload / download / delete; assert each completes with `ok=true`.  Existing s3-connection-test endpoint covers list; the executor task is the path that exercises PUT/GET/DELETE with the new `Content-Type` extra-header signing.
    - **Effort:** small (test infrastructure already in place; LocalStack setup is the bulk).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6).  New `test/dispatch/test_s3_roundtrip.py` — adhoc JCWF chains python-canary → s3 upload → download → delete (shared `working_directory`) against `my-s3` (minio).  6/6 PASS: all three S3 verbs signed-and-accepted live, exercising the new `m_ExtraHeadersToSign` `Content-Type` signing on PUT.  Connection-test list (GET) also `ok:true`.

13. **Live OAuth consent round-trip for SecureString `StoreTokens`** — added 2026-05-24 after item #2 closed.  Code-reviewed only this session.  The function-signature widening is byte-identical for storage but the SecureString end-to-end posture from consent-callback parse site → `TokenEntry` was not exercised live.
    - **Why it matters:** The threading shape mirrors Sitting 8e R1 (`PerformRefresh` snapshots) which is independently verified by the OAuth refresh heap-scan scenario, but the consent-callback path doesn't have its own scenario.
    - **Fix sketch:** Configure a real OAuth provider (OneDrive after PKCE handshake, Google Sheets via service-account JWT-bearer, etc.); complete the consent flow; verify tokens land in `m_Tokens` with SecureString-only storage.  Optional: extend the heap-scan audit with a `ScenarioStoreTokens` mirror of `ScenarioOAuthPostBody`.
    - **Effort:** small-medium (depends on a configured OAuth provider).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6) — live Google Sheets consent round-trip.  The callback's `StoreTokens` stored access + refresh + client_secret as `SecureString`; verified **0** plaintext `ya29.`/`1//` tokens across `security.txt` / `log.txt` / `keys.json.enc`, connection test `ok:true`.  The suggested heap-scan `ScenarioStoreTokens` fallback is **confounded and was NOT added**: `SecretRedactor::AddSecret` stores secrets as plaintext (`std::vector<std::string>` — it needs cleartext to redact log lines), so a post-`StoreTokens` scan always finds the nonce as an accepted residual, not a leak.  The live round-trip is the honest verification.

14. **Heartbeat handler accepts auth-failed `AuthResult`** — added 2026-05-24, uncovered while verifying item #4.  Pre-existing bug: `HandleMcpHeartbeatPost` checks only `!auth.has_value()`, not `auth->m_Error`.  `TryMcpAuth` returns `AuthResult{"invalid_token", "", ""}` on a bogus `mcp_*` token (logs `mcp_auth_failure` + calls `RecordAuthFailure` so the lockout fires correctly), but the heartbeat handler treats this as success → 200 + updates `m_McpLastHeartbeat`.
    - **Why it matters:** Staleness-alert suppression — any caller with a non-empty `mcp_*` Bearer header can pin `IsMcpConnected()` to true, defeating the alert gate the comment at the auth site explicitly warns about ("Without this gate, any unauthenticated caller could pin IsMcpConnected() to true and suppress staleness alerts").  The adversarial-flood protection (lockout counter) still works.
    - **Fix sketch:** Add `if (!auth->m_Error.empty()) { return MakeAuthErrorResponse(...); }` after the `!auth.has_value()` check in `HandleMcpHeartbeatPost`.
    - **Effort:** small (one-line fix + a test).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6).  Added the `!auth->m_Error.empty()` → 403 reject (no double-count; `TryMcpAuth` owns `RecordAuthFailure`).  Regression: `test_auth_mcp.py::test_heartbeat_rejects_bogus_mcp_token` (bogus `mcp_*` → 403, valid → 200); 103/103.

15. **Migrate remaining 12 cloud-task executors to emit typed `ConnectorErrorCode` on failure** — added 2026-05-24 after item #8 closed.  Today only `dbQueryCloudTaskExecutor.cpp` populates `TaskState::m_LastFailureCode` on its three cap-rejection sites.  The other 12 executors (azureBlob / email / gcs / gitHub / googleSheets / jira / oneDrive / polarionWrite / redmine / s3 / slack / snowflake) still hit the legacy untyped `RecordFailure(name)` path — which the breaker treats as connection-class by conservative default, so a string of expected-app-level failures from those executors (e.g. S3 PutObjectAccessDenied) would still trip the breaker Open.
    - **Why it matters:** The IsConnectionFailure policy benefits only the executors that emit typed codes.  Full coverage means every cloud executor classifies its failures into Network / Auth / Credential / ValueOutOfRange / etc., and the breaker can correctly distinguish "endpoint dead" from "user submitted a too-big upload".
    - **Fix sketch:** Per-executor, identify the app-level rejection branches that currently return false (size caps, attachment-count caps, role-rejection rejections from the provider) and set `taskState.m_LastFailureCode` to the appropriate `ConnectorErrorCode` before the `return false`.  New ConnectorErrorCode variants only if no existing one fits (today: `ValueOutOfRange` covers cap classes; `AuthFailure` covers 401/403; `HttpError` covers other 4xx — the latter could be split if a 4xx-vs-5xx distinction becomes load-bearing).  Companion: remove the vestigial `_warm_up_connection` calls in `test/hardening/test_negative_paths.py` tests 4.2/4.3 once db_query path is unambiguously fully migrated (today they're no-ops for the cap branches, but harmless defensive bridge while other executors haven't been migrated).
    - **Effort:** medium (~1 day for the 12 executors + the test cleanup; each is a few-line change but needs per-executor judgment on which branches qualify).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6).  All 12 executors (s3/gcs/azureBlob/oneDrive/email/slack/jira/redmine/gitHub/googleSheets/snowflake/polarionWrite) now set `taskState.m_LastFailureCode` at every `ExecuteCloud`-level `return false`: app-level cap/size/quota → `ValueOutOfRange` (the only breaker-behavioral code); curl-failure → `NetworkError`; 401/403 → `AuthFailure`; other 4xx/5xx → `HttpError`; bad params / path-confinement → `InvalidConfig`; SSRF gate → `InvalidEndpoint`.  Helper-internal `return false` (taskState out of scope) tagged at the caller.  Vestigial `_warm_up_connection` calls + helper removed from `test_negative_paths.py` 4.2/4.3.  Verified: `test_negative_paths.py` 55/55 (4.4 confirms 6 cap rejections leave the breaker Closed, `last_failure_code=value_out_of_range`).

16. **`AdhocWorkflowManager` reaper architecture cleanup** — added 2026-05-24 during item #7 investigation.  Two small architectural improvements surfaced while reading the code, neither correctness-load-bearing:
    - **(a) Separate `m_ReaperCvMutex` from `m_Mutex`.**  Currently `m_ReaperCv.wait_for` pairs with `m_Mutex`, the manager-wide data mutex shared with `Reap()` / `Stage()` / `OnRunCompleted()` / `GetUserDiskUsageBytes()`.  If `Stop` arrives while `Reap` is mid-directory-walk, the `notify_all` blocks behind Reap's lock until the walk finishes.  A dedicated CV mutex would let the notify proceed independently.  Marginal in practice (reaper is woken only at shutdown today, and Reap walks are typically <100 ms), but architecturally tidier.
    - **(b) First Reap on start, not after 60 s.**  `ReaperLoop` is `while (running) { wait_for(60s); Reap(); }` — on j9t restart, folders already past delete-at linger an extra 60 s before the first sweep.  Trivial reorder to `Reap(); wait_for(60s);` fixes it.  Visible behaviour change at restart (immediate cleanup of stale folders) but observable to operators who restart with stale runs in flight.
    - **Why it matters:** Both are post-1.0-grade polish.  Neither breaks anything today.  The decision rule (per `feedback_no_breadcrumbs`) is "bundle into a future sitting when adjacent reaper work happens" rather than carving out a dedicated sitting.
    - **Effort:** small (~1 h for both).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6).  Both landed: dedicated `m_ReaperCvMutex` (CV no longer shares the manager-wide `m_Mutex`, so a `Stop` notify can't queue behind a mid-flight `Reap` walk), and `ReaperLoop` reordered to `Reap(); wait_for(60s);`.

17. **`m_PersistedEmailWatermarks` map grows unboundedly across workflow removals** — added 2026-05-25 during item #10 Part 5 verification.  `uninstall_workflow` + `POST /api/workflows/reload` clears the in-memory `m_EmailWatchTriggers` entry but leaves the persisted-watermark entry in `m_PersistedEmailWatermarks` (keyed `workflowId|triggerId`).  The next save rewrites `queue/.email_watermarks.json` with all entries including the orphaned ones — visible today in the file after a test run with three terminated-workflow entries surviving.
    - **Why it matters:** Slow leak.  Entries are ~200 bytes each and real-world workflow removals are rare, so the practical bound is low.  But unbounded is unbounded — a long-lived j9t with frequent JCWF churn (CI-driven workflow regeneration, frequent rename cycles) would grow the file across months.  Pre-1.0 because we promised "persists across restart" without "persists across removal".
    - **Fix sketch:** Two options.  (a) At `ClearWorkflowTriggers` / `RemoveEmailWatchTrigger` (whichever ends up being the trigger-side removal site), drop the matching `m_PersistedEmailWatermarks` entry and call `SavePersistedEmailWatermarksLocked()`.  (b) At save time, prune entries whose `(workflowId, triggerId)` no longer correspond to a registered trigger — simpler but rewrites the file more often.  Option (a) is more surgical.  Companion: add a save-prune assertion to `test_email_watch_persistence.py` (after `uninstall_workflow`, expect the file to no longer reference that wfid).
    - **Effort:** small (~1 h including the test assertion).
    - **Status:** closed 2026-06-01 (Sitting 16 Part 6) — option (b).  Workflow removal goes through `ClearAll()` + `RegisterAll()` (the reload path), not a per-workflow `ClearWorkflowTriggers`, so option (a)'s removal-site hook wouldn't fire for a reload-based uninstall.  `SavePersistedEmailWatermarksLocked()` now prunes any persisted key with no matching live `m_EmailWatchTriggers` entry before writing — by the time a save fires (only after a poll), `RegisterAll` has repopulated the survivors, so orphans are dropped and survivors kept.  New `test_email_watch_persistence.py` **S6** scenario asserts an uninstalled workflow's watermark is pruned while a sibling's survives; 19/19.
