...
## Editor UX review → **see `editor-ux-refactor-plan.md`**

The UX review JC called for (2026-06-09) is written up as a full review + phased
refactor plan in **`doc/misc/editor-ux-refactor-plan.md`**. It synthesises the
F-1…F-42 findings into design principles (no free-text ids, edges carry data,
files as nodes, hide filesystem traversals, progressive disclosure, no-code
fan-out), reviews every inspector element against the "no-coding" goal, and
sequences the work into four phases. This Findings log remains the evidence the
plan cites.
