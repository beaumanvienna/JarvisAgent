...
1. **File exclusions** (`premake5.lua`) — entire modules (`code/backend/application/assistant/**`, `code/backend/application/web/aiJcwfService.*`) are excluded from Engine builds via `removefiles`.
...
Beyond registered workflows, external MCP agents can submit one-shot JCWFs via `POST /api/workflows/run-adhoc`. The `AdhocWorkflowManager` (`code/backend/application/workflow/adhocWorkflowManager.h`) stages each submission into a per-user folder — `_adhoc/<user_slug>/<timestamp>_<counter>_del-<delete-at>/` — with a `meta.json` (owner attribution) and, at completion, a `manifest.json` (file inventory). Scripts referenced by the JCWF must already exist under `scripts/`; the submit handler verifies every `params.command` / `params.module` up front and rejects missing ones with `400 missing_scripts`.
...
- `GET /api/scripts` — the `ScriptCatalog` (`code/backend/application/workflow/scriptCatalog.h`) scans `scripts/` at startup and parses each script's `@jarvis-script` metadata header (short / params / outputs / description) so agents can discover what's pre-deployed before composing a JCWF.

Authorization: operators can read their own runs (handler compares `auth.m_User == info->m_User`); admins can read any run (with an `admin_cross_user_read` audit line). Path traversal (`..`, absolute paths, symlinks) is rejected before the filesystem is touched. A background reaper thread sweeps runs whose `delete-at` has passed; empty user-slug directories are pruned at the same time so the `_adhoc/` root stays tidy.
...
- **`AiInvocation`** (`code/backend/application/workflow/aiInvocation.h`) is a pure data struct — no inheritance. Carries interface name, optional model override, `AiSettings` (temperature, seed, max_tokens), messages, optional `output_schema`, `StructuredMode`, timeout, retry policy, queue folder, prob name, optional chunk index/count. This envelope is the unambiguous source of truth at runtime.
...
- **`IAuthSigner`** (`code/backend/engine/curlWrapper/authSigner.h`) is the single authentication strategy. `[[nodiscard]] bool Apply(QueryData const&, vector<string>& publicHeaders, SecureString& secretHeader, string& errorMessage) const` is called by both `CurlWrapper::Query` (sync) and `LiveTransport` (async parallel, hosted under `CurlMultiDispatcher`) — no per-style branching in the curl layer.  Output split: `publicHeaders` (non-secret: Content-Type, version, signed-not-secret headers), `secretHeader` (caller-owned SecureString filled via `SecureString::Format` for Bearer / x-api-key / x-goog-api-key / api-key, and for SigV4 X-Amz-Security-Token when an STS session token is present).  Returns `false` on validation failure (empty/whitespace credentials, missing SigV4 region/secret); upstream callers emit a structured ERROR with run context (`m_QuotaKey`, `m_CancelKey`, URL) for the dashboard's run analyzer rather than letting the request go out unsigned and bounce off the provider as an opaque 401. Concrete signers: `BearerSigner`, `XGoogApiKeySigner`, `AnthropicXApiKeySigner`, `AzureApiKeySigner`, `SigV4Signer`. SigV4 is hand-rolled on OpenSSL (`code/backend/engine/curlWrapper/awsSigV4.{h,cpp}`) — no `aws-sdk-cpp` dependency.  A 4-test startup self-test in debug builds checks SHA256-empty + AWS-published key-derivation hex + determinism + a full-chain known-answer signature; intermediate signing keys are RAII-cleansed via `OPENSSL_cleanse`, and the input phase reads `m_SecretAccessKey` / `m_SessionToken` as `SecureString` (no `std::string` materialisation).  See `doc/cyber security.md` "IAuthSigner Security", "CurlMultiDispatcher Security", "CurlWrapper Security", "SigV4 Signing Security", and "SecureString-only HTTP path" for the full per-subsystem guarantees (response-body cap, exception-safe C-boundary callbacks, Submit-after-shutdown race closure, fail-path log enrichment, IPv6 host extraction, etc.).
...
`doc/jcwf.schema.json` and `doc/jcwf_generation_guide.md` are embedded into the binary at premake time (`embedAsHeader` in `premake5.lua` → `code/backend/application/json/jcwfSchema.generated.h` / `jcwfGenerationGuide.generated.h`) as `kJcwfSchemaJson` / `kJcwfGenerationGuide` constants. This means:
...
The `max_concurrency` multiplier exists because serializing backends (local ollama / llama.cpp / vLLM on a single GPU) process requests one at a time — a request at position N in the queue takes N × single-stream wall-clock. Scaling the budget by the interface's configured ceiling guarantees the timeout reflects worst-case queue position regardless of backend shape; truly-parallel cloud backends (OpenAI / Anthropic / etc.) over-allocate timeout harmlessly. No per-user knob to tune. Defaults clamp at `max_seconds: 1800` (30 min) — past that point a request is almost certainly hung rather than legitimately progressing.
...
**Cascade cancellation.** When a workflow run terminates (failed / cancelled / stopped), `WorkflowRuntimeManager` calls `AiRequestPool::CancelRequestsForRun(runId)` once. The pool walks pending entries, finds matches by run id, and forwards each to `CurlMultiDispatcher::CancelByCancelKey(...)`. On the I/O thread, `DrainPendingCancellations` runs in three phases: **(1)** under the relevant dispatcher locks, it partitions the inbox / retry queue / active set, moves matching entries out, and **collects** their user callbacks into a local vector — no callback fires while any dispatcher mutex is held; **(2)** transport-side silent cleanup (`LiveTransport::CancelByCancelKey` → `curl_multi_remove_handle` + `curl_easy_cleanup`) runs between the lock release and the callback fire; **(3)** every collected callback fires with `Fail(CURLE_ABORTED_BY_CALLBACK, "request cancelled (run terminated)")` outside every dispatcher lock. The same callback-outside-lock discipline applies to `OnTransportComplete` for natural (non-cancel) completions. Holding `m_DebugMutex` while firing user callbacks previously caused an AB-BA deadlock against any concurrent `GetDebugSnapshot` caller (which acquires `m_InboxMutex` first then `m_DebugMutex`), since the callback re-entered the dispatcher via the cap-pin path and re-needed `m_InboxMutex`. The lightweight `CurlMultiDispatcher::GetCurrentConcurrencyCap(quotaKey)` exists for the cap-pin read so callbacks don't need `GetDebugSnapshot` at all — it locks only `m_DebugMutex`. Validated under load: 125 in-flight ollama requests aborted in <1 ms during a workflow cancel, with `ai_calls_inflight` decrementing to 0 within 2 s and `/api/debug/signals` staying HTTP 200 throughout. Counter exposed as `dispatcher_total_cancelled` on `/api/debug/signals`.
...
| **Per-user folder namespace for adhoc** | `_adhoc/<user_slug>/<run>/` where `user_slug` is `<body>_<8 hex chars of SHA-256(user)>` | Authorisation primitive is the original user identity (`m_User`), never a derived slug — distinct users that character-sanitise to the same body still get distinct filesystem dirs via the hash suffix. Handler identity check *and* hash-suffixed filesystem path prefix form defence in depth. |
...
| **Hand-edited `config.json` scalar persistence is find-replace + JSON-escape + simdjson tripwire + atomic rename; encrypted stores are serialize→encrypt→atomic-write** | `HandleConfigSettingsPut` persists the `config.json`-resident **scalar** fields (max threads, verbose, file-size, jcwf batch size, use_bash) by reading the file as text, depth-aware top-level-key replace, simdjson re-parse tripwire, then atomic-rename via `WebServerHelpers::WriteTextFileAtomic`. The **AI-routing table and cloud connections no longer live in `config.json`** — they moved to the master-password-encrypted `API.json.enc` / `connections.json.enc`, owned by `ApiInterfaceManager` / `CloudConnectionManager`, which serialize→`KeyEncryption::Encrypt`→`AtomicWriteFile` through the shared `EncryptedJsonStore` helpers; every such mutation is gated by a master-password re-auth (`CheckMasterPasswordReauth`). | A full parse-and-reserialize of `config.json` would lose user comments / field ordering / layout — it is a hand-edited document, so scalar edits keep the find-replace path (`JsonHelper::EscapeJsonString` closes the quote/backslash/newline corruption window; the simdjson tripwire fails loudly on a bad splice; the atomic rename means a 5xx leaves the file unchanged). Routing config is *not* hand-edited and *is* security-sensitive, so it lives encrypted at rest, dashboard-managed, never spliced into plaintext — closing the tamper/exfiltration vector where editing a plaintext `url`+`key_name` could redirect a live credential. |
...
| **Per-attempt timeout = `CURLOPT_TIMEOUT_MS` from a size-aware budget; runtime `WaitingExternal` timeout dropped for `ai_call`** | `AiRequestPool::Submit` computes `timeoutMs = ((in_tok/1000)*per_in + (out_tok/1000)*per_out + fixed) × max_concurrency × safety, clamped[min,max]` from `api->m_RateLimit.m_RequestBudget` and writes it into `QueryData::m_TimeoutMs`. The dispatcher hands it to curl; `WorkflowRuntimeManager::TimeoutWaitingExternalTasks` no longer fires for `ai_call` tasks | Curl already counts only in-flight time (not queue waits) and resets per attempt because each retry creates a fresh easy handle. The pre-1.0 dual-timeout (per-attempt deadline in `AiRequestPool` + 300 s `WaitingExternal` kill in the runtime) used to murder legitimately-slow Sonnet calls when the runtime's clock fired first. Single owner, single clock, ~130 lines of deadline-bookkeeping deleted. The `× max_concurrency` factor was added when local-LLM use (ollama / llama.cpp / vLLM on a single GPU) exposed that the budget needs to cover worst-case backend queue position, not just single-stream wall-clock; cloud backends over-allocate timeout harmlessly. Defaults clamp at `max_seconds: 1800` — past that, a request is almost certainly hung. |
...
| **`AdhocWorkflowManager` reaper uses condition_variable; lifecycle mutex serializes Start/Stop; teardown is explicit from `WebServer::SignalStop`** | `m_ReaperCv` lets `StopReaperThread` wake the reaper out of its `wait_for(60s)` immediately instead of sleeping out the interval.  The CV pairs with a dedicated `m_ReaperCvMutex` (not the manager-wide `m_Mutex` shared with `Reap` / `Stage` / `OnRunCompleted`), so a `Stop` notify never queues behind a mid-flight `Reap` directory walk.  `ReaperLoop` sweeps first (`Reap(); wait_for(60s);`) so a restart cleans already-expired folders at once rather than after the first interval.  `m_ReaperLifecycleMutex` serializes Start/Stop — atomic `m_ReaperRunning` alone doesn't make the std::thread member assignment safe under concurrent calls.  `WebServer::SignalStop` invokes `m_AdhocManager->StopReaperThread()` directly (right after the WRM observer detach), so reaper teardown runs inside `engine.cpp`'s 3-s shutdown watchdog window; the destructor still calls it as an idempotent safety net. | Pre-fix the reaper was only torn down via `~AdhocWorkflowManager` in the `unique_ptr<Application>` destruction at end of `main`, AFTER the shutdown watchdog had been diffused.  A regression that reintroduced a long `sleep_for` in `StopReaperThread` would silently stall exit by up to 60 s with no `[shutdown watchdog] timeout expired` line and no `_exit(EXIT_FAILURE)`.  Lifting the stop into `SignalStop` brings the teardown inside the watchdog-covered phase so any future stall becomes a loud force-exit instead of a silent slow exit.  (Earlier pre-fix used a 1 s busy-poll loop — the CV pattern + lifecycle mutex matches the rest of the codebase's reaper / worker-thread idioms.) |
