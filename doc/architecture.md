...
**Cascade cancellation.** When a workflow run terminates (failed / cancelled / stopped), `WorkflowRuntimeManager` calls `AiRequestPool::CancelRequestsForRun(runId)` once. The pool walks pending entries, finds matches by run id, and forwards each to `CurlMultiDispatcher::CancelByCancelKey(...)` — which (on the I/O thread, where curl handle mutations are safe) drops matching entries from the inbox and retry queue and aborts in-flight `m_Active` handles via `curl_multi_remove_handle` + `curl_easy_cleanup`. Cancelled callbacks fire with `Fail(CURLE_ABORTED_BY_CALLBACK, "request cancelled (run terminated)")`. Validated under load: 126 in-flight Anthropic requests aborted in <1 ms when an upstream task failed, halting token burn that would otherwise have continued for orphaned generations. Counter exposed as `dispatcher_total_cancelled` on `/api/debug/signals`.
...
| **AI-task output writes are atomic via temp-then-rename** | Both `WriteTextFile` implementations — `AiCallTaskExecutor::WriteTextFile` (public static, used by inline queue-file writes + ConvertWithMarkitdown destination + provider-sidecar JSON writes) and the anonymous-namespace `WriteTextFile` in `aiRequestPool.cpp` (used by the AI-call output write path) — open `<final>.tmp.<counter>` (where `counter` is a per-implementation `static std::atomic<uint64_t>`), write, close via scope, then `fs::rename` to the final path.  On rename failure the temp file is best-effort cleaned up; error messages surface only the basename to API consumers (server-side `LOG_APP_ERROR` carries the full path).  Internal best-effort signals (e.g. the Test-interface short-circuit's `OnOutputFileCreated` notification) acknowledge `[[nodiscard]]` discards explicitly with `(void)` and a comment.  See `application/file/README.md` "Atomic-rename writes" for the codebase-wide use-site list. | Pre-fix used a single open→write→close stream.  A SIGKILL or disk-full mid-write would leave a truncated partial at the canonical path; downstream `OnOutputFileCreated` (which keys completion off the file content) would parse the partial as a malformed completion signal, the workflow runtime would attribute the failure to the task, and the operator would see `task succeeded but output unparseable` — a confusing failure mode that doesn't surface the actual cause.  Atomic rename gives every reader either the previous-version contents or the new-version contents, never a torn write.  The atomic counter (rather than `getpid()`) keeps temp names unique across concurrent writers without pulling platform-specific PID headers. |
| **Dashboard WebSocket reconnect uses exponential backoff** | `dashboard/ui/src/hooks/useWebSocket.ts` reconnect timer starts at 2 s, doubles on each consecutive failed connect (capped at 30 s), and resets to base on successful `onopen`.  The backend `Authenticate()` rate-limit + lockout flow stays unchanged — the fix is purely client-side. | Pre-fix used a fixed 2 s reconnect.  When a dashboard tab opened before the user logged in (no session cookie), every 2 s it would generate one `[security] auth_failure reason=missing_credential` + one `[security] ws_upgrade_rejected` line in the security log.  A long-open-tab scenario produced thousands of warning lines per hour, drowning the genuine security signal.  Exponential backoff settles at ~30 s between attempts after ~5 failures; once the user logs in and `onopen` fires, the next reconnect (e.g. after a server restart) starts back at base. |
