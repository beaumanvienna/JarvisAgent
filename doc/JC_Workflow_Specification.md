...
- **Completion callback:** When the run finishes, if `callbackUrl` was provided in the
  ...
  the run result).
- **Callback URL safety gate (SSRF defense):** `callbackUrl` MUST use the `https://`
  scheme.  The host is DNS-resolved before any payload is built; any address resolving
  to a loopback / RFC 1918 / link-local / unique-local / multicast / cloud-metadata
  range (incl. `169.254.169.254`) is refused with an ERROR-logged
  `[callback] refused completion callback ...` line.  Plain `http://`, redirects, and
  non-HTTPS protocols are also refused at the curl layer (`CURLOPT_FOLLOWLOCATION = 0`,
  `CURLOPT_PROTOCOLS_STR = "https"`).
- **Opt-out for output content:** by default the `tasks` object includes each succeeded
  task's `outputs` (slot name → value, with file content read up to 65 KB per slot).
  To exclude that — e.g. when outputs may contain PII or secrets — set
  `callback_include_outputs` in the run context (or webhook body's `context`) to
  `"false"` / `"0"` / `"no"`.  The state + per-task state + error messages are still
  delivered; only the outputs payload is stripped.
...
- **Module allowlist.** The runtime additionally validates `params.module` against `ScriptRegistry::FindByModulePath` before calling `PyImport_ImportModule`. Only modules whose `.py` file under `scripts/` carries a `# @jarvis-script` header (and is therefore tracked by the registry) are importable. Unregistered names — including standard-library modules such as `os`, `subprocess`, `ctypes` — are rejected with a structured error before any Python state is touched. The registry is populated at startup by `ScriptRegistry::ScanDirectory(scripts/)` and refreshed live via file-watch events; new scripts become importable as soon as the file watcher picks them up.
- If the module cannot be imported, the function is missing / not callable, or the module is not in the registry, the task fails and the error message is recorded and logged.
...
**Engine-level fan-out cap:** Independent of the per-filter `max_items`, the engine config `max_per_item_fan_out` (default 10000; 0 = unlimited; see `doc/jarvisagent.md`) caps the children spawned by any single per_item filter evaluation.  Operators can lower this to bound per-tenant cost; if a filter returns more items than the cap allows, the parent task fails with `per_item filter '<id>' returned N items, exceeds max_per_item_fan_out=<cap>` and the run is failed.  This is the operator-side backstop — JCWFs cannot raise it.
...
1. **AI inventory** — the AI JCWF generation pipeline consumes the registry as a Markdown table (see §11.6) so generated workflows reference scripts that actually exist with the right call signatures.
2. **Runtime import allowlist for `python` tasks** — `PythonEngine::ExecuteWorkflowTaskOnWorker` calls `ScriptRegistry::FindByModulePath(params.module)` before `PyImport_ImportModule`. Modules that lack the `@jarvis-script` header (and therefore aren't in the registry) are rejected before any Python state is touched, including standard-library modules like `os`, `subprocess`, and `ctypes`. **Adding the header is therefore mandatory for any new Python script that a workflow needs to import** — without it, the import is rejected at runtime even though the file exists on disk and `sys.path` would otherwise locate it.
