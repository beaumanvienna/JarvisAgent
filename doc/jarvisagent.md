...
- **`"API interfaces"`** — (array) List of AI provider configurations. Each entry is an object with:
  ...
  - **`"fixture_path"`** — (string, optional unless `is_mock: true`) Path to the on-disk fixture file MockTransport reads as the response body.  The file must be a full API-shaped response (e.g. an OpenAI chat completion JSON for `api_type: "API1"`); the configured `api_type`'s ReplyParser parses it identically to a real response.  Hardening: paths are resolved against the project root via `ConfineUnderProjectRoot` and rejected if they escape (absolute outside the root, symlink target outside, `..` traversal that lands outside).  Fixture size is capped at 10 MiB; oversized files fail closed.  An optional sibling `<fixture>.meta.json` overrides HTTP status + headers — `http_status` must be in `[200, 599]`; only `Content-Type` and `Retry-After` headers are allowlisted (others dropped with WARN).  PROV sidecar carries `"mocked": true` + the resolved `fixture_path` so post-mortem tooling distinguishes mock dispatches from live ones.
