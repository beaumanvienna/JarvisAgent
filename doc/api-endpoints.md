...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/debug/signals` | Live engine introspection — AI dispatcher, controllers, workflow runs, websocket, python pool, uptime, plus cloud-surface security counters (`cloud_dns_resolved_ip_rejections`, `cloud_endpoint_ssrf_rejections`, `cloud_credential_crlf_rejections`, `cloud_input_validation_rejections`, `cloud_postgres_invalid_sslmode_rejections`, `cloud_postgres_forbidden_param_rejections`), AI-dispatch URL-policy counters (`url_policy_rejections`, `credentialed_plaintext_http_rejections`, `ai_dispatch_nonloopback_http_rejections`), and a `last_mock_signatures` array surfacing recent MockTransport auth-signer captures (FIFO ring, cap 32; each entry `{cancel_key, quota_key, headers}` — consumed by `test/dispatch/test_bedrock_sigv4.py` for the hermetic SigV4 KAT).  Returns 404 on Release builds. |
